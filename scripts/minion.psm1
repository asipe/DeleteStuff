...
function Bootstrap() {
  .\thirdparty\nuget\nuget.exe install .\src\DeleteStuff.Nuget.Packages\common\packages.config -OutputDirectory .\thirdparty\packages\common -ExcludeVersion | Write-Host
  .\thirdparty\nuget\nuget.exe install .\src\DeleteStuff.Nuget.Packages\net-4.5\packages.config -OutputDirectory .\thirdparty\packages\net-4.5 -ExcludeVersion | Write-Host
}
...
function RunUnitTestsVS() {
  ...
  .\thirdparty\packages\common\nunit.runners\tools\nunit-console.exe .\src\DeleteStuff.UnitTests\bin\debug\DeleteStuff.UnitTests.dll /nologo /framework:net-4.5 | Write-Host
  CheckLastExitCode
...
function RunUnitTests() {
  ...
  .\thirdparty\packages\common\nunit.runners\tools\nunit-console.exe .\debug\net-4.5\DeleteStuff.UnitTests\DeleteStuff.UnitTests.dll /nologo /framework:net-4.5 | Write-Host
  CheckLastExitCode
  Write-Host -ForegroundColor Cyan '----------------------------------'
}
...
function Build() {
  C:\Windows\Microsoft.NET\Framework64\v4.0.30319\MSBuild.exe .\src\DeleteStuff.Build\DeleteStuff.proj /ds /maxcpucount:6 | Write-Host
  CheckLastExitCode
}
...
function BuildNugetPackages() {
  ...
  Copy-Item .\debug\net-4.5\TODO.core\TODO.core.dll .\nugetworking\core\lib\net45 -Verbose
  Copy-Item .\src\TODO.nuget.specs\TODO.core.dll.nuspec .\nugetworking\core -Verbose
  

  thirdparty\nuget\nuget.exe pack .\nugetworking\core\TODO.core.dll.nuspec -OutputDirectory .\nugetworking\core | Write-Host
  CheckLastExitCode
}
...
function PushNugetPackages() {
  ...
  if ($response -eq 'YES') {
    ...
    thirdparty\nuget\nuget.exe push .\nugetworking\core\DeleteStuff.Core.1.0.0.0.nupkg | Write-Host
    CheckLastExitCode

    thirdparty\nuget\nuget.exe push .\nugetworking\testing\DeleteStuff.Testing.1.0.0.0.nupkg | Write-Host
    CheckLastExitCode
  } else {
